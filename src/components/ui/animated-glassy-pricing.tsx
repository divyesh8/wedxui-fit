...
const ShaderCanvas = () => {
  ...
  useEffect(() => {
    ...
    const updateColor = () => {
      ...
      setBackgroundColor(isDark ? [0.04, 0.04, 0.06] : [1.0, 1.0, 1.0]);
    };
    updateColor();
  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = `
      ...
      void main(){
        ...
        vec3 foregroundColor=vec3(v.x*0.8+0.3, v.y*0.4, .7-v.y*v.x);
...
export interface PricingCardProps {
  ...
  currency?: string;
  /** Billing period suffix, e.g. '/mo' or '/yr'. Defaults to '/mo'. */
  period?: string;
  /** Optional struck-through original price for launch offers. */
  originalPrice?: string;
  /** Optional savings badge, e.g. '₹100 OFF'. */
  badge?: string;
  features: string[];
  buttonText: string;
  isPopular?: boolean;
...
export const PricingCard = ({
  ...
  const cardClasses = `
    ...
    ${isPopular ? 'scale-105 relative ring-2 ring-wed-purple/40 dark:from-wed-purple/20 dark:to-wed-blue/10 dark:border-wed-purple/40 shadow-[0_0_40px_rgba(176,38,255,0.25)]' : ''}
  `;
  const buttonClasses = `
    mt-auto w-full py-2.5 rounded-xl font-semibold text-[14px] transition font-sans
    ${buttonVariant === 'primary'
      ? 'bg-gradient-purple hover:brightness-110 text-white'
  ...
  return (
    <div className={cardClasses.trim()}>
      {isPopular && (
        <div className="absolute -top-4 right-4 px-3 py-1 text-[12px] font-semibold rounded-full bg-wed-purple text-white shadow-lg">
          Most Popular
        </div>
      )}
      <div className="mb-3">
      ...
      <div className="my-6 flex items-baseline gap-2 flex-wrap">
        {originalPrice && (
          <span className="text-[22px] font-extralight text-foreground/40 line-through font-display">
        ...
        <span className="text-[14px] text-foreground/70 font-sans">{period}</span>
        {badge && (
          <span className="ml-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-wed-lime/20 text-wed-lime">
            {badge}
          </span>
        )}
      </div>
      ...
      <ul className="flex flex-col gap-2 text-[14px] text-foreground/90 mb-6 font-sans">
        {features.map((feature, index) => (
          <li key={index} className="flex items-center gap-2">
            <CheckIcon className="text-wed-lime w-4 h-4 flex-shrink-0" /> {feature}
          </li>
        ))}
      </ul>
    ...
    <div className="bg-background text-foreground min-h-screen w-full overflow-x-hidden">
      ...
      <main className="relative w-full min-h-screen flex flex-col items-center justify-center px-4 py-16">
        <div className="w-full max-w-5xl mx-auto text-center mb-14">
          <h1 className="text-[44px] md:text-[60px] font-extralight leading-tight tracking-[-0.03em] bg-clip-text text-transparent bg-gradient-to-r from-slate-900 via-wed-purple to-wed-blue dark:from-white dark:via-wed-purple dark:to-wed-blue font-display">
            {title}
          </h1>
